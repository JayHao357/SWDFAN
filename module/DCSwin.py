...
class DCFAM(nn.Module):
    def __init__(self, encoder_channels=(96, 192, 384, 768), atrous_rates=(6, 12)):
        ...
        self.downsample = ConvBN(encoder_channels[0], encoder_channels[0], stride=2)

        self.ca = SharedChannelAttention()
        self.pa_3 = SharedSpatialAttention(in_places=encoder_channels[2])
        self.pa_2 = SharedSpatialAttention(in_places=encoder_channels[1])
        # self.ppm = PPMBilinear()
        self.aspp = ASPP()


        self.scse_1 = scSE(in_channels=encoder_channels[0])
        self.scse_2 = scSE(in_channels=encoder_channels[1])
        self.scse_3 = scSE(in_channels=encoder_channels[2])
        self.scse_4 = scSE(in_channels=encoder_channels[3])

    def forward(self, x1, x2, x3, x4):

        out4 = self.aspp(self.conv4(self.scse_4(x4)))

        # x4 = self.conv4(self.scse_4(x4))
        # out4 = self.ppm(self.conv4(self.scse_4(x4)))
        down_2 = self.scse_2(x2)
        down_3 = self.scse_3(x3)
        x1 = self.downsample(self.conv1(self.scse_1(x1)))
        down_3 = F.interpolate(down_3, (64, 64), mode='bilinear', align_corners=True)
        out4 = F.interpolate(out4, (64, 64), mode='bilinear', align_corners=True)
        # down_2 = self.downsample223(down_2)
        out = torch.cat((x1, down_2, down_3, out4), dim=1)
        return out


class ASPP(nn.Module):
    def __init__(self, in_channel=768, depth=256):
...
class PPMBilinear(nn.Module):
    def __init__(self, num_classes=7, fc_dim=768,
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.segmentation_head = nn.Sequential(
            ConvBNReLU(928, encoder_channels[0]),
            Conv(encoder_channels[0], num_classes, kernel_size=1),
        )
        # self.segmentation_head = nn.Sequential(
        #     ConvBNReLU(679, encoder_channels[0]),
        #     Conv(encoder_channels[0], num_classes, kernel_size=1),
        # )
...
class DCSwin(nn.Module):
    ...
    def forward(self, x):
        ...
        e1 = self.decoder(x1, x2, x3, x4)
        e2 = self.decoder(x1, x2, x3, x4)
        if self.training:
            return e1, e2, x4
        else:
            x1 = F.interpolate(e1, (512, 512), mode='bilinear', align_corners=True)
            x2 = F.interpolate(e2, (512, 512), mode='bilinear', align_corners=True)
            return (x1.softmax(dim=1) + x2.softmax(dim=1)) / 2


def dcswin_base(pretrained=True, num_classes=7, weight_path='pretrain_weights/stseg_base.pth'):
...
def dcswin_small(pretrained=True, num_classes=8, weight_path='pretrain_weights/stseg_small.pth'):
