...
def get_console_file_logger(name, level=logging.INFO, logdir='./baseline'):
    ...
    return logger
def loss_calc_swin(pred, label, reduction='mean', multi=False):
    loss_s = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=-1),
                       DiceLoss(smooth=0.05, ignore_index=-1), 1.0, 1.0)
    if multi is True:
        loss = 0
        num = 0
        for p in pred:
...
COLOR_MAP = OrderedDict(
    cultivated_land=(61, 153, 245),
    wood_land=(83, 178, 79),
    grass=(132, 240, 127),
    river=(250, 59, 38),
    bare_land=(116, 216, 244),
    building=(184, 161, 238),
    road=(114, 114, 114),
    artificial_land=(255, 255, 255)
...
def generate_pseudo(model, target_loader, save_dir, n_class=8, pseudo_dict=dict(), logger=None):
